...
void CoreKeyMouse()
{
	...
	if ( key[GLUT_KEY_UP   ] ) screen.pos=screen.pos+movZ * (0.03 * deltaTime);
	if ( key[GLUT_KEY_DOWN ] ) screen.pos=screen.pos-movZ * (0.03 * deltaTime);
	if ( key[GLUT_KEY_LEFT ] ) screen.pos=screen.pos+movX * (0.03 * deltaTime);
	if ( key[GLUT_KEY_RIGHT] ) screen.pos=screen.pos-movX * (0.03 * deltaTime);
	if ( key['s'] ) CoreScreenshot();
