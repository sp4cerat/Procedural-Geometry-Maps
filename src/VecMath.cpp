...
vec3f vec3f::random(){
	static vec3f rnd;
	rnd.x=random_float()*2-1;
	rnd.y=random_float()*2-1;
	rnd.z=random_float()*2-1;
	rnd.normalize();
	return rnd;
}
...
{ 
	...
	float len = 1.0f / (float)sqrt(square); 
...
//	a1 a2 x - a1 c2 y = a1 d2 - a1 b2
  ...
  //  y =  ( a1 d2 - a1 b2 - a2 d1 + a2 b1 ) / ( a2 c1 - a1 c2 )
	...
	float div1 =  delta1.y*delta2.x - delta1.x*delta2.y;
	float div2 =  delta2.y*delta1.x - delta2.x*delta1.y;
