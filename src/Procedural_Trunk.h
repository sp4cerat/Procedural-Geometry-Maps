...
//		int t;loop(t,-10,10){ displace+=get_liana(a+30+t+t*b,0.02)*15;}
//		int t;loop(t,-10,10){ displace=get_max(get_liana(a+30+t+t*b,0.02)*15,displace);}
	...
	vec3f trunk_c_out(float a, float b)
	{
		vec3f c;
		c.x=0.20;
		c.y=1.0;
		...
		{
			static vec3f rn;
			rn.x=float(i^74)*0.01;
			rn.y=float(300-(i^3))*0.01;
			rn.z=float(i^23)*0.01;
			rn.random01_fxyz();
			float v=get_liana2(a+20+sin(b*4*rn.x+rn.y*10)*0.2+b*(rn.y-0.5)*5+rn.z*20,rn.x*0.01);
